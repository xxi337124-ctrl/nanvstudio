...
import { Camera3DParams, DEFAULT_CAMERA_PARAMS } from './types/camera3d';
import {
...
const ExpandedView = ({ media, onClose }: ExpandedViewProps) => {
  ...
  const handlePrev = useCallback(
    (e?: React.MouseEvent) => {
      ...
      if (hasMultiple) {
        setCurrentIndex(
          prev => (prev - 1 + media.images.length) % media.images.length
        );
      }
    },
    [hasMultiple, media]
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex items-center justify-center transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${visible ? 'bg-black/90 backdrop-blur-xl' : 'bg-transparent pointer-events-none opacity-0'}`}
      onClick={handleClose}
    >
      <div
        className={`relative w-full h-full flex items-center justify-center p-8 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${visible ? 'scale-100 opacity-100' : 'scale-90 opacity-0'}`}
      ...
      <button
        onClick={handleClose}
        className="absolute top-6 right-6 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white backdrop-blur-md transition-colors z-[110]"
      >
        <X size={24} />
      </button>
    </div>
...
export const App = () => {
  ...
  const deleteNodes = useCallback(
    (ids: string[]) => {
      ...
      setConnections(p =>
        p.filter(c => !ids.includes(c.from) && !ids.includes(c.to))
      );
      setSelectedNodeIds([]);
    },
    [saveHistory]
  ...
  const addNode = useCallback(
    (
            ...
            : type === NodeType.VIDEO_ANALYZER
              ...
              : type === NodeType.AUDIO_GENERATOR
                ...
                : type.includes('IMAGE')
                  ...
                  : 'gemini-3-pro-preview',
        generationMode:
          type === NodeType.VIDEO_GENERATOR ? 'DEFAULT' : undefined,
        ...initialData,
      };
      ...
      const safeX =
        x !== undefined ? x : (-pan.x + window.innerWidth / 2) / scale - 210;
      const safeY =
  ...
  // 创建 3D 镜头控制节点
  const handleCreate3DControlNode = useCallback(
    (sourceNodeId: string, sourceImage: string, sourcePrompt?: string) => {
      ...
      const newNodeX = sourceNode.x + 500;
      ...
      const newNode: AppNode = {
        ...
        data: {
          ...
          camera3DParams: DEFAULT_CAMERA_PARAMS,
        },
        inputs: [sourceNodeId],
      };
  ...
  // 从3D镜头控制节点生成新视角图片
  const handleGenerateFrom3DControlNode = useCallback(
    async (controlNodeId: string, params: { horizontal: number; vertical: number; zoom: number }) => {
      const controlNode = nodes.find(n => n.id === controlNodeId);
      if (!controlNode || !controlNode.data.sourceImage) return;

      const sourceImage = controlNode.data.sourceImage;
      const sourcePrompt = controlNode.data.sourcePrompt;

      // 查找已存在的生成节点数量
      const existingGeneratedNodes = nodes.filter(
        n => n.inputs?.includes(controlNodeId)
      );

      // 计算新节点位置
      let newNodeY = controlNode.y;
      let newNodeX = controlNode.x + 400;

      if (existingGeneratedNodes.length > 0) {
        // 如果已有生成节点,新节点放在最后一个下方
        const lastNode = existingGeneratedNodes[existingGeneratedNodes.length - 1];
        newNodeY = lastNode.y + (lastNode.height || 360) + 20;
      }

      // 生成提示词
      const hLabel = params.horizontal === 0 ? 'front' : params.horizontal === 90 ? 'side' : params.horizontal === 180 ? 'back' : '3/4';
      const vLabel = params.vertical === 0 ? 'eye' : params.vertical > 0 ? 'high' : 'low';
      const zIndex = params.zoom < 3 ? 'close' : params.zoom < 7 ? 'medium' : 'wide';
      const enhancedPrompt = `${sourcePrompt || ''} ${hLabel} ${vLabel} ${zIndex} shot`.trim();
      ...
      const newNode: AppNode = {
        ...
        y: newNodeY,
        width: 420,
        title: `H:${params.horizontal.toFixed(0)}° V:${params.vertical.toFixed(0)}° Z:${params.zoom.toFixed(1)}x`,
        ...
        inputs: [controlNodeId],
      };

      setNodes(prev => [...prev, newNode]);
      setConnections(prev => [...prev, { from: controlNodeId, to: newNode.id }]);
      ...
      try {
        ...
        setNodes(prev =>
          prev.map(n =>
            n.id === newNode.id
              ? { ...n, data: { ...n.data, image: res[0], images: res }, status: NodeStatus.SUCCESS }
              : n
          )
        );

        setAssetHistory(h => [
          { id: `a-${Date.now()}`, type: 'image' as const, src: res[0], title: newNode.title, timestamp: Date.now() },
          ...h,
        ]);
      } catch (error: any) {
        setNodes(prev =>
          prev.map(n =>
            n.id === newNode.id
              ? { ...n, data: { ...n.data, error: error?.message || '生成失败' }, status: NodeStatus.ERROR }
              : n
          )
        );
      }
  ...
  // 监听3D镜头控制节点的生成请求
  useEffect(() => {
    const handleGenerateFrom3DControl = (e: Event) => {
      const customEvent = e as CustomEvent<{
        nodeId: string;
        params: { horizontal: number; vertical: number; zoom: number };
      }>;
      const { nodeId, params } = customEvent.detail;

      const controlNode = nodes.find(n => n.id === nodeId);
      if (!controlNode || !controlNode.data.sourceImage) return;

      handleGenerateFrom3DControlNode(nodeId, params);
    };

    window.addEventListener('generateFrom3DControl', handleGenerateFrom3DControl);
    return () => {
      window.removeEventListener('generateFrom3DControl', handleGenerateFrom3DControl);
    };
  }, [nodes, handleGenerateFrom3DControlNode]);
  ...
  const handleSketchResult = (
    ...
    if (type === 'image') {
      addNode(NodeType.IMAGE_GENERATOR, centerX, centerY, {
        image: result,
        prompt,
        status: NodeStatus.SUCCESS,
      });
    } else {
      addNode(NodeType.VIDEO_GENERATOR, centerX, centerY, {
        videoUri: result,
        prompt,
        status: NodeStatus.SUCCESS,
      });
    ...
    try {
      ...
      if (res.isFallbackImage) {
        handleAssetGenerated(
          'image',
          res.uri,
          'Smart Sequence Preview (Fallback)'
        );
      } else {
  ...
  const handleWheel = (e: React.WheelEvent) => {
    if (e.ctrlKey || e.metaKey) {
      ...
      setPan(p => ({
        x: p.x - (x - p.x) * (scaleDiff / scale),
        y: p.y - (y - p.y) * (scaleDiff / scale),
      }));
      setScale(newScale);
    } else {
    ...
    (e: MouseEvent) => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        if (dragGroupRef.current) {
          const { id, startX, startY, mouseStartX, mouseStartY, childNodes } =
          ...
          if (childNodes.length > 0) {
            setNodes(prev =>
              prev.map(n => {
                ...
                return child
                  ? { ...n, x: child.startX + dx, y: child.startY + dy }
                  : n;
              })
            );
          }
        ...
        if (
          draggingNodeId &&
          dragNodeRef.current &&
          dragNodeRef.current.id === draggingNodeId
        ) {
          const {
            startX,
            startY,
          ...
          // Snap Logic
          const SNAP = UI.SNAP_THRESHOLD / scale;
          const myL = proposedX;
          ...
          nodesRef.current.forEach(other => {
            ...
            if (!snappedX) {
              if (Math.abs(myL - otherBounds.x) < SNAP) {
                proposedX = otherBounds.x;
                snappedX = true;
              } else if (Math.abs(myL - otherBounds.r) < SNAP) {
                proposedX = otherBounds.r;
                snappedX = true;
              } else if (Math.abs(myR - otherBounds.x) < SNAP) {
                proposedX = otherBounds.x - nodeWidth;
                snappedX = true;
              } else if (Math.abs(myR - otherBounds.r) < SNAP) {
                proposedX = otherBounds.r - nodeWidth;
                snappedX = true;
              } else if (
                Math.abs(myC - (otherBounds.x + otherBounds.width / 2)) < SNAP
              ) {
                proposedX =
                  otherBounds.x + otherBounds.width / 2 - nodeWidth / 2;
                snappedX = true;
              }
            }
            if (!snappedY) {
              if (Math.abs(myT - otherBounds.y) < SNAP) {
                proposedY = otherBounds.y;
                snappedY = true;
              } else if (Math.abs(myT - otherBounds.b) < SNAP) {
                proposedY = otherBounds.b;
                snappedY = true;
              } else if (Math.abs(myB - otherBounds.y) < SNAP) {
                proposedY = otherBounds.y - nodeHeight;
                snappedY = true;
              } else if (Math.abs(myB - otherBounds.b) < SNAP) {
                proposedY = otherBounds.b - nodeHeight;
                snappedY = true;
              } else if (
                Math.abs(myM - (otherBounds.y + otherBounds.height / 2)) < SNAP
              ) {
                proposedY =
                  otherBounds.y + otherBounds.height / 2 - nodeHeight / 2;
                snappedY = true;
              }
            }
          });
          ...
          setNodes(prev =>
            prev.map(n =>
              n.id === draggingNodeId ? { ...n, x: proposedX, y: proposedY } : n
            )
          );
          ...
          setNodes(prev =>
            prev.map(n =>
              n.id === resizingNodeId
                ? {
                    ...n,
                    width: Math.max(360, initialSize.width + dx),
        ...
        if (enclosed.length > 0) {
          ...
          if (freeNodes.length > 0) {
            const fMinX = Math.min(...freeNodes.map(n => n.x)),
              fMinY = Math.min(...freeNodes.map(n => n.y)),
              fMaxX = Math.max(...freeNodes.map(n => n.x + (n.width || 420))),
              fMaxY = Math.max(...freeNodes.map(n => n.y + 320));
            setGroups(prev => [
              ...prev,
              {
                id: generateGroupId(),
        ...
        for (const other of otherNodes) {
          ...
          if (isOverlapping) {
            ...
            if (minOverlap === overlapLeft) {
              draggedNode.x =
                otherBounds.x - myBounds.width - UI.COLLISION_PADDING;
            } else if (minOverlap === overlapRight) {
              draggedNode.x = otherBounds.r + UI.COLLISION_PADDING;
            } else if (minOverlap === overlapTop) {
              draggedNode.y =
                otherBounds.y - myBounds.height - UI.COLLISION_PADDING;
            } else if (minOverlap === overlapBottom) {
              draggedNode.y = otherBounds.b + UI.COLLISION_PADDING;
            }
  ...
  const handleNodeAction = useCallback(
    async (id: string, promptOverride?: string) => {
      ...
      setNodes(p =>
        p.map(n => (n.id === id ? { ...n, status: NodeStatus.WORKING } : n))
      );
      ...
      try {
        ...
        if (node.type === NodeType.IMAGE_GENERATOR) {
          ...
          const isStoryboard =
            /分镜|storyboard|sequence|shots|frames|json/i.test(prompt);
          ...
          if (isStoryboard) {
            try {
              ...
              if (storyboard.length > 1) {
                ...
                const groupWidth =
                  Math.min(storyboard.length, COLUMNS) * childWidth +
                  (Math.min(storyboard.length, COLUMNS) - 1) * gapX +
                  groupPadding * 2;
                const groupHeight =
                  totalRows * childHeight +
                  (totalRows - 1) * gapY +
                  groupPadding * 2;
          ...
          const vid =
            node.data.videoUri ||
            inputs.find(n => n?.data.videoUri)?.data.videoUri;
  ...
  const saveGroupAsWorkflow = (groupId: string) => {
    ...
    const connectionsInGroup = connections.filter(
  ...
  const handleCanvasDrop = (e: React.DragEvent) => {
    ...
    if (workflowId && workflows) {
      ...
      if (wf) {
        ...
        const width =
          Math.max(...wf.nodes.map(n => n.x + (n.width || 420))) - minX;
      ...
      try {
        ...
        if (asset && asset.type) {
          if (asset.type === 'image')
            ...
            addNode(NodeType.VIDEO_GENERATOR, dropX - 210, dropY - 180, {
              videoUri: asset.src,
            });
        }
        return;
  ...
  return (
    <div className="w-screen h-screen overflow-hidden bg-[#0a0a0c]">
      ...
      >
        ...
        <div
          className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] z-50 pointer-events-none ${nodes.length > 0 ? 'opacity-0 scale-105' : 'opacity-100 scale-100'}`}
        >
          {/* ... (Welcome Screen) ... */}
          <div className="flex flex-col items-center justify-center mb-10 select-none animate-in fade-in slide-in-from-bottom-8 duration-1000">
            <div className="relative">
              <h1
                className="text-6xl md:text-8xl font-black tracking-tighter text-white px-4 pb-2 relative"
                style={{
                  textShadow:
                    '0 0 40px rgba(255, 255, 255, 0.3), 0 0 80px rgba(147, 197, 253, 0.2), 0 0 120px rgba(139, 92, 246, 0.15)',
                }}
              >
                NOVA STUDIO
              </h1>
              <div className="absolute -inset-10 bg-gradient-to-r from-violet-500/15 via-purple-500/20 to-blue-500/15 blur-[80px] opacity-30 pointer-events-none"></div>
            </div>
          ...
          <div
            className={`flex items-center gap-1.5 p-1.5 rounded-[18px] bg-[#0a0a0f]/60 backdrop-blur-2xl border border-white/10 shadow-2xl ${nodes.length > 0 ? 'pointer-events-none' : 'pointer-events-auto'}`}
          >
            <button
              onClick={() => addNode(NodeType.IMAGE_GENERATOR)}
              className="flex items-center gap-2.5 px-5 py-3 rounded-[14px] bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white transition-all border border-white/10 hover:border-cyan-400/50 hover:shadow-[0_0_20px_rgba(34,211,238,0.3)] group shadow-sm hover:-translate-y-0.5 duration-300 backdrop-blur-md"
            >
              <ImageIcon
                size={16}
            ...
            <button
              ...
              className="flex items-center gap-2.5 px-5 py-3 rounded-[14px] bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white transition-all border border-white/10 hover:border-purple-400/50 hover:shadow-[0_0_20px_rgba(168,85,247,0.3)] group shadow-sm hover:-translate-y-0.5 duration-300 backdrop-blur-md"
            >
              <Film
                size={16}
                className="text-zinc-500 transition-colors group-hover:text-purple-400"
              />
              <span className="text-[13px] font-medium tracking-wide">
                文生视频
              </span>
            ...
            <button
              onClick={() =>
                addNode(NodeType.VIDEO_GENERATOR, undefined, undefined, {
                  generationMode: 'FIRST_LAST_FRAME',
                })
              }
              className="flex items-center gap-2.5 px-5 py-3 rounded-[14px] bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white transition-all border border-white/10 hover:border-emerald-400/50 hover:shadow-[0_0_20px_rgba(52,211,153,0.3)] group shadow-sm hover:-translate-y-0.5 duration-300 backdrop-blur-md"
            >
              <Link
                size={16}
        ...
        <input
          ...
          className="hidden"
          accept="video/*"
          onChange={e => handleReplaceFile(e, 'video')}
        />
        <input
          type="file"
          ...
          className="hidden"
          accept="image/*"
          onChange={e => handleReplaceFile(e, 'image')}
        />
        ...
        >
          ...
          {groups.map(g => (
            <div
              key={g.id}
              className={`absolute rounded-[32px] border transition-all ${draggingGroup?.id === g.id || draggingNodeParentGroupId === g.id ? 'duration-0' : 'duration-300'} ${selectedGroupId === g.id ? 'border-cyan-500/30 bg-cyan-500/5' : 'border-white/10 bg-white/5'}`}
              ...
              (() => {
                ...
                if (connectionStart.id === 'smart-sequence-dock') {
                  ...
                  const startNode = nodes.find(
                    n => n.id === connectionStart.id
                  );
                  if (!startNode) return null;
                  const startHeight =
                    startNode.height || getApproxNodeHeight(startNode);
          ...
          {nodes.map(node => {
            ...
            return (
              ...
              onGenerate3DView={(sourceNodeId, sourceImage, sourcePrompt) => {
                handleCreate3DControlNode(sourceNodeId, sourceImage, sourcePrompt);
              }}
                  ...
                  if (pGroup) {
                    siblingNodeIds = nodes
                      .filter(other => {
                        ...
                        return (
                          ocx > pGroup.x &&
                          ocx < pGroup.x + pGroup.width &&
                          ocy > pGroup.y &&
                          ocy < pGroup.y + pGroup.height
                        );
                      })
                      .map(s => s.id);
                  }
              ...
              inputAssets={node.inputs
                ...
                .filter(
                  n =>
                    n &&
                    (n.data.image || n.data.videoUri || n.data.croppedFrame)
                )
                .slice(0, 6)
                .map(n => ({
                  id: n!.id,
                  type:
                    n!.data.croppedFrame || n!.data.image ? 'image' : 'video',
                  src:
                    n!.data.croppedFrame || n!.data.image || n!.data.videoUri!,
                }))}
              onInputReorder={(nodeId, newOrder) => {
                ...
                if (node) {
                  setNodes(prev =>
                    prev.map(n =>
                      n.id === nodeId ? { ...n, inputs: newOrder } : n
                    )
                  );
                }
              }}
              ...
              isGroupDragging={activeGroupNodeIds.includes(node.id)}
            />
            );
          })}
          ...
          {selectionRect && (
            <div
              ...
              style={{
                left:
                  (Math.min(selectionRect.startX, selectionRect.currentX) -
                    pan.x) /
                  scale,
                top:
                  (Math.min(selectionRect.startY, selectionRect.currentY) -
                    pan.y) /
                  scale,
                width:
                  ...
                  scale,
                height:
                  Math.abs(selectionRect.currentY - selectionRect.startY) /
                  scale,
              }}
            />
          )}
                ...
                {(() => {
                  ...
                  if (targetNode) {
                    const isVideo =
                      ...
                      targetNode.type === NodeType.VIDEO_ANALYZER;
                    const isImage =
                      targetNode.type === NodeType.IMAGE_GENERATOR ||
                      targetNode.type === NodeType.IMAGE_EDITOR;
                    if (isVideo || isImage) {
                      return (
                        <button
                          ...
                          onClick={() => {
                            replacementTargetRef.current = contextMenu.id;
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        addNode(
                          t,
                          (contextMenu.x - pan.x) / scale,
                          (contextMenu.y - pan.y) / scale
                        );
                        setContextMenu(null);
                      }}
                    >
            ...
            {contextMenuTarget?.type === 'group' && (
              <>
                ...
                <button
                  ...
                  onClick={() => {
                    setGroups(p => p.filter(g => g.id !== contextMenu.id));
                    setContextMenu(null);
                  }}
                >
                  {' '}
              ...
              <button
                ...
                onClick={() => {
                  setConnections(prev =>
                    prev.filter(
                      c =>
                        c.from !== contextMenuTarget.from ||
                        c.to !== contextMenuTarget.to
                    )
                  );
                  setNodes(prev =>
                    prev.map(n =>
        ...
        {croppingNodeId && imageToCrop && (
          <ImageCropper
            ...
            onConfirm={b => {
              handleNodeUpdate(croppingNodeId, { croppedFrame: b });
              setCroppingNodeId(null);
              setImageToCrop(null);
            }}
          />
        ...
        <SonicStudio
          ...
          onGenerate={(src, prompt) =>
            handleAssetGenerated('audio', src, prompt)
          }
        />
        <SettingsModal
        ...
        <SidebarDock
          ...
          onHistoryItemClick={item => {
            const type = item.type.includes('image')
              ? NodeType.IMAGE_GENERATOR
              : NodeType.VIDEO_GENERATOR;
            const data =
              item.type === 'image'
        ...
        <div className="absolute bottom-8 right-8 flex items-center gap-3 px-4 py-2 bg-[#1c1c1e]/80 backdrop-blur-2xl border border-white/10 rounded-full shadow-2xl z-50 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="flex items-center gap-2 min-w-[100px]">
            <input
              ...
              className="w-24 h-1 bg-white/20 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:shadow-lg hover:[&::-webkit-slider-thumb]:scale-125 transition-all"
            />
            <span
              className="text-[10px] font-bold text-slate-400 w-8 text-right tabular-nums cursor-pointer hover:text-white"
              onClick={() => setScale(1)}
              title="Reset Zoom"
            >
          ...
          <button
            ...
            className="p-1.5 text-slate-400 hover:text-white transition-colors rounded-full hover:bg-white/10 ml-2 border-l border-white/10 pl-3"
            title="适配视图"
          >
