...
export const processDefaultVideoGen = async (
  ...
  if (imageInput) {
    inputImageForGeneration =
      imageInput.data.croppedFrame || imageInput.data.image || null;
  }
...
export const processSceneDirector = async (
  ...
  if (videoInputNode && videoInputNode.data.videoUri) {
    try {
      ...
      upstreamContextStyle = await analyzeVideo(
        vidData,
        'Analyze the visual style, lighting, composition, and color grading briefly.',
        'gemini-2.5-flash'
      );
    } catch (_e) {
  ...
  if (node.data.croppedFrame) {
    ...
    if (cropSource) {
      ...
      if (!inputImageForGeneration && videoInputNode) {
        try {
          inputImageForGeneration = await extractLastFrame(
            videoInputNode.data.videoUri!
          );
        } catch (_e) {
          // Intentionally empty
        }
  ...
  if (inputImageForGeneration) {
    try {
      ...
      console.warn(
        'SceneDirector: Restoration failed, using original crop',
        reframeErr
      );
    }
  }
...
export const processCharacterRef = async (
  ...
  const characterImage =
    ...
    inputs.find(n => n.data.image)?.data.image ||
    null;
