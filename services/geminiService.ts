...
const getClient = () => {
  if (!process.env.API_KEY) {
    throw new Error(
      'API Key is missing. Please select a paid API key via the Google AI Studio button.'
    );
  }
...
const getPolloKey = async () => {
  // Try new secure storage first, fall back to legacy
  const secureKey = await getSecureApiKey('pollo');
  if (secureKey) return secureKey;
  return getLegacyApiKey('pollo');
};

const getOpenRouterKey = async () => {
  // Try new secure storage first, fall back to legacy
  const secureKey = await getSecureApiKey('openrouter');
  if (secureKey) return secureKey;
  return getLegacyApiKey('openrouter');
};

// Helper to check if we should use OpenRouter
const shouldUseOpenRouter = async (model: string): Promise<boolean> => {
  const openRouterKey = await getOpenRouterKey();
  return (
    !!openRouterKey &&
    (model.includes('gemini-2.5-flash') || model.includes('openrouter'))
  );
};
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (isOverloaded && i < maxRetries - 1) {
        ...
        console.warn(
          `API Overloaded (503/429). Retrying in ${delay}ms... (Attempt ${i + 1}/${maxRetries})`
        );
        await wait(delay);
        continue;
      }
...
const combineBase64Chunks = (
  ...
  for (let i = 0; i < wavFile.length; i += chunk) {
    binary += String.fromCharCode.apply(
      null,
      Array.from(wavFile.subarray(i, i + chunk))
    );
  }
...
const convertImageToCompatibleFormat = async (
  base64Str: string
): Promise<{ data: string; mimeType: string; fullDataUri: string }> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onerror = e =>
      reject(new Error('Image conversion failed for Veo compatibility'));
    img.src = base64Str;
  });
};
...
You are NOVA STUDIO AI, an expert multimedia creative assistant.
...
export const generateImageFromText = async (
  ...
  if (await shouldUseOpenRouter(model)) {
    console.log(
      '🚀 Using OpenRouter for image generation with Gemini 2.5 Flash Image'
    );
    try {
      const images = await generateImageWithOpenRouter(
        prompt,
        options.aspectRatio || '1:1',
        inputImages[0] // Use first image as reference if available
      );
      return images;
    } catch (e: any) {
      console.error(
        'OpenRouter image generation failed, falling back to Google AI SDK:',
        e
      );
  ...
  const effectiveModel = model.includes('imagen')
  ...
  const selectedAspectRatio = options.aspectRatio || '1:1';
  ...
  try {
    ...
    if (response.candidates?.[0]?.content?.parts) {
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData && part.inlineData.data) {
          ...
          if (!validation.valid) {
            console.warn(
              `⚠️ 图片比例不匹配！期望: ${selectedAspectRatio}, 实际: ${validation.actualRatio}`
            );
            console.log(
              `📐 实际尺寸: ${validation.dimensions.width}x${validation.dimensions.height}`
            );
            console.log(`✂️ 自动裁剪到 ${selectedAspectRatio}...`);

            try {
              const croppedImage = await cropImageToAspectRatio(
                imageData,
                selectedAspectRatio
              );
            ...
            console.log(`✅ 图片比例正确: ${validation.actualRatio}`);
            images.push(imageData);
          }
        }
      }
    ...
    if (images.length === 0) {
      throw new Error(
        'No images generated. Safety filter might have been triggered.'
      );
    }
...
export const generateVideo = async (
  ...
  // Check if we should use OpenRouter for video generation
  if (await shouldUseOpenRouter(model)) {
    console.log(
      '🚀 Using OpenRouter for video generation with Gemini 2.5 Flash Image'
    );
    try {
      const videoUri = await generateVideoWithOpenRouter(prompt, {
        aspectRatio: options.aspectRatio || '16:9',
      });
      return { uri: videoUri, isFallbackImage: false, uris: [videoUri] };
    } catch (e: any) {
      console.error(
        'OpenRouter video generation failed, falling back to Google AI SDK:',
        e
      );
      // Fall through to original implementation
    }
  ...
  const qualitySuffix =
    ', cinematic lighting, highly detailed, photorealistic, 4k, smooth motion, professional color grading';
  const enhancedPrompt = prompt + qualitySuffix;

  // --- Model Selection & Resolution ---
  // Default Veo Pro to 1080p if not specified
  const resolution =
    options.resolution || (model.includes('pro') ? '1080p' : '720p');
  ...
  if (
    ...
    for (const ref of referenceImages) {
      ...
      refsPayload.push({
        image: { imageBytes: c.data, mimeType: c.mimeType },
        referenceType: 'ASSET',
      });
    }
  ...
  try {
    ...
    if (validUris.length === 0) {
      ...
      throw (
        firstError?.reason ||
        new Error('Video generation failed (No valid URIs).')
      );
    }
    ...
    try {
      ...
      throw new Error(
        'Video generation failed and Image fallback also failed: ' +
          getErrorMessage(e)
      );
    }
  }
...
export const analyzeVideo = async (
  ...
  if (videoBase64OrUrl.startsWith('data:')) {
    const mime =
      videoBase64OrUrl.match(/^data:(video\/\w+);base64,/)?.[1] || 'video/mp4';
    ...
    throw new Error(
      'Direct URL analysis not implemented in this demo. Please use uploaded videos.'
    );
  }
...
export const orchestrateVideoPrompt = async (
  ...
  parts.push({
    text: `Create a single video prompt that transitions between these images. User Intent: ${userPrompt}`,
  });
...
export const compileMultiFramePrompt = (frames: any[]) => {
  ...
  return (
    'A sequence showing: ' +
    frames.map(f => f.transition?.prompt || 'scene').join(' transitioning to ')
  );
};
...
export const generateAudio = async (
  ...
  if (referenceAudio) {
    const mime =
      referenceAudio.match(/^data:(audio\/\w+);base64,/)?.[1] || 'audio/wav';
  ...
  // Config for TTS
  const voiceName =
    options?.persona?.label === 'Deep Narrative' ? 'Kore' : 'Puck'; // Mapping example
...
export const transcribeAudio = async (audioBase64: string): Promise<string> => {
  const ai = getClient();
  const mime =
    audioBase64.match(/^data:(audio\/\w+);base64,/)?.[1] || 'audio/wav';
