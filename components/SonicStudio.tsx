...
const VOICE_PERSONAS = [
  {
    label: '深沉叙述 (Narrator)',
    desc: 'Deep, resonant male voice, slow pacing, storytelling style.',
    gender: 'Male',
  },
  {
    label: '活力解说 (Energetic)',
    desc: 'High energy, fast paced, enthusiastic YouTuber style.',
    gender: 'Any',
  },
  {
    label: '知性新闻 (News)',
    desc: 'Professional, articulate, neutral tone, broadcast standard.',
    gender: 'Female',
  },
  {
    label: '动漫少女 (Anime)',
    desc: 'High pitched, cute, expressive, "kawaii" aesthetic.',
    gender: 'Female',
  },
  {
    label: '电影旁白 (Epic)',
    desc: 'Gravelly, dramatic, movie trailer voice, intense.',
    gender: 'Male',
  },
  {
    label: '慈祥长者 (Elder)',
    desc: 'Warm, shaky, wise, slow speaking grandmother/grandfather.',
    gender: 'Any',
  },
];
...
export const SonicStudio: React.FC<SonicStudioProps> = ({
  ...
  const [transcribeFileName, setTranscribeFileName] = useState<string | null>(
    null
  );
  ...
  const handleGenerateClick = async () => {
    ...
    try {
      const audioUri = await generateAudio(
        ...
        { persona: selectedPersona, emotion: selectedEmotion }
      );
      ...
      setCurrentTrack({
        ...
        src: audioUri,
        title:
          textPrompt.substring(0, 30) + (textPrompt.length > 30 ? '...' : ''),
        timestamp: Date.now(),
      });
    } catch (e) {
      console.error(e);
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-1 h-48">
            {history.length === 0 ? (
              ...
              history.map(item => (
                ...
                >
                  <div
                    className={`w-6 h-6 rounded bg-gradient-to-br ${getRandomCover(item.id)} flex items-center justify-center shrink-0`}
                  >
                    <Mic2 size={10} className="text-white/70" />
                  </div>
                  <span
                    className={`text-xs truncate ${currentTrack?.id === item.id ? 'text-cyan-400' : 'text-slate-400'}`}
                  >
                    {item.title}
                  </span>
                </div>
            ...
            {activeTab === 'factory' && (
              <>
                <div className="space-y-1 pl-12">
                  {' '}
                  {/* Added left padding for title due to close button */}
                  ...
                  <p className="text-slate-400 text-sm">
                    Create realistic voiceovers, clone voices, and generate
                    emotional speech.
                  </p>
                </div>
                      ...
                      >
                        ...
                        {referenceAudio ? (
                          <>
                            ...
                            <span className="text-xs font-medium text-cyan-200 truncate max-w-[80%]">
                              {referenceFileName}
                            </span>
                            <button
                              onClick={e => {
                              ...
                              className="absolute top-2 right-2 p-1.5 rounded-full bg-black/40 hover:bg-red-500/80 text-white/50 hover:text-white transition-colors"
                            >
                              <X size={10} />
                            </button>
                          </>
                          ...
                          <>
                            ...
                            <span className="text-[9px] text-slate-600 text-center px-4">
                              AI 将模仿此声音朗读文本 (Approximate)
                            </span>
                          </>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {EMOTIONS.map(emo => (
                          <button
                            ...
                            className={`px-3 py-2 rounded-lg text-xs font-medium text-left transition-all ${selectedEmotion.value === emo.value ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-white/5 text-slate-400 hover:bg-white/10 border border-transparent'}`}
                          >
                            {emo.label}
                          </button>
                        ))}
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-6">
                    <div className="relative group flex-1">
                      ...
                      <div className="relative bg-[#1c1c1e] border border-white/10 rounded-2xl p-5 shadow-2xl flex flex-col h-full min-h-[240px]">
                        <textarea
                          className="w-full flex-1 bg-transparent text-lg text-slate-200 placeholder-slate-600 focus:outline-none resize-none font-medium leading-relaxed"
                        ...
                        <div className="flex justify-between items-center mt-4 pt-4 border-t border-white/5">
                          <div className="flex items-center gap-2">
                            {selectedPersona && (
                              <div className="px-2 py-1 bg-purple-500/20 border border-purple-500/30 rounded text-[10px] text-purple-300 flex items-center gap-1">
                                <User size={10} /> {selectedPersona.label}
                              </div>
                            )}
                            {referenceAudio && (
                              <div className="px-2 py-1 bg-cyan-500/20 border border-cyan-500/30 rounded text-[10px] text-cyan-300 flex items-center gap-1">
                                <FileAudio size={10} /> Custom Clone
                              </div>
                            )}
                          </div>
                          <button
                            ...
                            className={`px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg transition-all flex items-center gap-2 ${!textPrompt.trim() || isGenerating ? 'bg-white/5 text-slate-500 cursor-not-allowed' : 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white hover:scale-105 hover:shadow-cyan-500/25'}`}
                          >
                            {isGenerating ? (
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {VOICE_PERSONAS.map(persona => (
                          <button
                            ...
                            className={`p-3 rounded-xl border text-left transition-all group ${selectedPersona?.label === persona.label ? 'bg-purple-500/10 border-purple-500/50' : 'bg-white/5 border-white/5 hover:border-white/20 hover:bg-white/10'}`}
                          >
                            <div className="flex justify-between items-start mb-1">
                              <span
                                className={`text-xs font-bold ${selectedPersona?.label === persona.label ? 'text-purple-400' : 'text-slate-300'}`}
                              >
                                {persona.label}
                              </span>
                            </div>
            ...
            {activeTab === 'transcribe' && (
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-300 pl-12">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-400 text-sm">
                    Accurately transcribe audio files into text using Gemini
                    2.5.
                  </p>
                </div>
                  ...
                  {transcribeFile ? (
                    <>
                      ...
                      <button
                        ...
                        className="absolute top-4 right-4 p-2 bg-black/40 hover:bg-red-500/20 text-white/50 hover:text-red-400 rounded-full"
                      >
                        <X size={16} />
                      </button>
                    </>
                ...
                {transcript && (
                  <div className="bg-[#1c1c1e] border border-white/10 rounded-2xl p-6 relative">
                    <button
                      ...
                      className="absolute top-4 right-4 p-2 text-slate-500 hover:text-white bg-black/20 hover:bg-black/40 rounded-lg transition-colors"
                      title="复制"
                    >
                      <FileText size={14} />
                    </button>
            ...
            {activeTab === 'live' && (
              <div className="flex flex-col items-center justify-center gap-10 py-10 animate-in fade-in slide-in-from-bottom-4 duration-300 pl-12">
                <div className="space-y-2 text-center">
                  ...
                  <p className="text-slate-400 text-sm">
                    Bidirectional real-time conversation with Gemini 2.0 Flash
                    Exp.
                  </p>
                </div>
                ...
                <div className="relative w-64 h-64 flex items-center justify-center">
                  ...
                  {isLiveActive && (
                    <>
                      <div
                        className="absolute inset-0 bg-cyan-500/20 rounded-full animate-ping opacity-20"
                        style={{ animationDuration: '2s' }}
                      />
                      <div
                        className="absolute inset-4 bg-purple-500/20 rounded-full animate-ping opacity-20"
                        style={{
                  ...
                  {/* Status Circle */}
                  <div
                    className={`relative z-10 w-48 h-48 rounded-full flex flex-col items-center justify-center transition-all duration-500 border-4 ${isLiveActive ? 'bg-gradient-to-br from-cyan-900/50 to-purple-900/50 border-cyan-500 shadow-[0_0_50px_rgba(6,182,212,0.3)]' : 'bg-[#1c1c1e] border-white/10'}`}
                  >
                    <Activity
                      size={48}
                      className={
                    ...
                    />
                    <span
                      className={`text-xs font-bold mt-4 uppercase tracking-widest ${isLiveActive ? 'text-cyan-200' : 'text-slate-600'}`}
                    >
                      {liveStatus}
                    </span>
                  </div>
                ...
                <p className="text-[10px] text-slate-500 max-w-sm text-center">
                  Uses Gemini Live API (WebSocket). Please allow microphone
                  access. Ensure you are in a quiet environment for best
                  results.
                </p>
              </div>
            )}
        ...
        {(activeTab === 'factory' ||
          ...
          <div className="h-24 bg-[#121214]/90 backdrop-blur-xl border-t border-white/10 flex items-center px-8 gap-8 relative z-20">
            ...
            <div className="w-64 flex items-center gap-4">
              <div
                className={`w-14 h-14 rounded-lg bg-gradient-to-br ${currentTrack ? getRandomCover(currentTrack.id) : 'from-slate-700 to-slate-800'} shadow-lg flex items-center justify-center`}
              >
                {currentTrack ? (
                  <Mic2 size={24} className="text-white/80" />
                ) : (
                  <Disc size={24} className="text-slate-500" />
                )}
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-sm font-bold text-white truncate">
                  {currentTrack?.title || 'Ready to speak'}
                </span>
            ...
            <div className="flex-1 flex flex-col items-center gap-2 max-w-2xl mx-auto">
              <div className="flex items-center gap-6">
                <button
                  ...
                  className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${currentTrack ? 'bg-white text-black hover:scale-110' : 'bg-white/10 text-slate-600'}`}
                >
                  {isPlaying ? (
            ...
            <div className="w-64 flex items-center justify-end gap-4">
              <div className="flex items-center gap-2 group/vol">
                ...
                <div className="w-20 h-1 bg-white/10 rounded-full relative cursor-pointer">
                  <div
                    className="absolute top-0 left-0 h-full bg-slate-400 group-hover/vol:bg-white rounded-full"
