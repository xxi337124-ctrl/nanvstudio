...
export const VideoModeSelector: React.FC<VideoModeSelectorProps> = ({
  currentMode,
  onSelect,
}) => {
  const modes = [
    {
      ...
      icon: Film,
      title: '剧情延展 (StoryContinuator)',
    },
    {
      mode: 'FIRST_LAST_FRAME' as const,
      icon: Link,
      title: '收尾插帧 (FrameWeaver)',
    },
    { mode: 'CUT' as const, icon: Scissors, title: '局部分镜 (SceneDirector)' },
    {
      mode: 'CHARACTER_REF' as const,
      icon: ScanFace,
      title: '角色迁移 (CharacterRef)',
    },
  ];
  ...
  return (
    ...
    >
      {hoverTime !== null && duration > 0 && (
        <div
          className="absolute top-0 bottom-0 w-0.5 bg-cyan-400 z-10 shadow-[0_0_8px_rgba(34,211,238,0.8)]"
          style={{ left: `${(hoverTime / duration) * 100}%` }}
        />
      )}
      <div className="w-full text-center text-[9px] text-slate-500 font-bold tracking-widest pointer-events-none">
        Scene Director Timeline
      </div>
    </div>
  );
};
