...
export const ImageCropper: React.FC<ImageCropperProps> = ({
  ...
  const handleConfirm = () => {
    if (!imgRef.current || !crop || crop.width === 0) {
      onConfirm(imageSrc);
      return;
    }
    ...
    <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center animate-in fade-in duration-300">
      ...
      >
        <img
          ref={imgRef}
          src={imageSrc}
          className="max-w-full max-h-[65vh] object-contain block opacity-50"
          draggable={false}
        />
          ...
          >
            ...
            <div className="absolute inset-0 border-2 border-cyan-400 z-10 pointer-events-none">
              <div className="absolute inset-0 grid grid-cols-3 grid-rows-3 opacity-40">
                ...
                <div className="border-r border-white/50" />
                <div className="col-span-3 border-b border-white/50 -mt-[33%]" />
                <div className="col-span-3 border-b border-white/50 mt-[33%]" />
              </div>
            </div>
            ...
            {['nw', 'ne', 'sw', 'se'].map(h => (
              <div
                ...
                onMouseDown={e =>
                  handleMouseDown(e, 'resize', h as ResizeHandle)
                }
              />
            ))}
      ...
      <div className="flex flex-col items-center gap-6 mt-8 w-full max-w-2xl px-4">
        ...
        <div className="flex gap-4">
          <button
            onClick={onCancel}
            className="px-6 py-2.5 rounded-full bg-white/5 hover:bg-white/10 text-white text-xs font-medium transition-colors border border-white/5"
          >
            取消
          </button>
          <button
