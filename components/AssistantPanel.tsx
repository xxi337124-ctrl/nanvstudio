  ...
  lines.forEach((line, index) => {
    ...
    if (line.startsWith('# ')) {
      elements.push(
        <h1
          key={key}
          className="text-base font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 mt-5 mb-3 border-b border-white/10 pb-2"
        >
          {line.replace(/^#\s/, '')}
        </h1>
      );
    ...
    if (line.startsWith('## ')) {
      elements.push(
        <h2
          key={key}
          className="text-sm font-bold text-white mt-4 mb-2 flex items-center gap-2"
        >
    ...
    if (line.startsWith('### ') || line.startsWith('#### ')) {
      ...
      elements.push(
        <h3
          key={key}
          className="text-xs font-bold text-cyan-300 mt-3 mb-1 uppercase tracking-wider"
        >
          {content}
        </h3>
      );
    ...
    if (trimmed.startsWith('* ') || trimmed.startsWith('- ')) {
      ...
      elements.push(
        <div
          key={key}
          className="flex gap-2 ml-1 mb-1.5 items-start group/list"
        >
    ...
    if (/^\d+\.\s/.test(trimmed)) {
      ...
      elements.push(
        <div key={key} className="flex gap-2 ml-1 mb-1.5 items-start">
          <span className="text-xs font-mono text-cyan-500/80 mt-[2px] shrink-0">
            {num}.
          </span>
          <div className="text-[13px] leading-relaxed text-slate-300 flex-1">
            {parseInlineStyles(content)}
    ...
    if (trimmed.startsWith('> ')) {
      ...
      elements.push(
        <div
          key={key}
          className="pl-3 border-l-2 border-cyan-500/30 italic text-slate-400 my-2 text-xs"
        >
          {parseInlineStyles(content)}
        </div>
      );
    ...
    elements.push(
      <div
        key={key}
        className="text-[13px] leading-relaxed text-slate-300 mb-1"
      >
...
export const AssistantPanel: React.FC<AssistantPanelProps> = ({
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: '你好！我是您的创意助手。今天想创作些什么？' },
  ]);
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar bg-[#0a0a0c]/50">
        {messages.map((m, i) => (
          ...
          >
            <div
              className={`flex flex-col max-w-[92%] gap-1.5 ${m.role === 'user' ? 'items-end' : 'items-start'}`}
            >
              {/* Role Label */}
              <div className="flex items-center gap-2 px-1">
                {m.role === 'model' && (
                  <span className="text-[10px] font-bold text-cyan-500/80 uppercase tracking-wider">
                    NOVA STUDIO AI
                  </span>
                )}
                {m.role === 'user' && (
              ...
              <div className="group relative transition-all w-full">
                ...
                >
                  {m.role === 'model' ? (
                    ...
                    <p className="leading-6 text-[13px] whitespace-pre-wrap">
                      {m.text}
                    </p>
                  )}
        ...
        {isLoading && (
          <div className="flex justify-start w-full animate-in fade-in slide-in-from-bottom-2">
            <div className="flex flex-col gap-2 max-w-[85%]">
              <span
                className={`text-[10px] font-bold uppercase tracking-wider px-1 ${isThinkingMode ? 'text-indigo-400' : 'text-cyan-500/80'}`}
              >
                {isThinkingMode ? 'Deep Thinking' : 'Thinking'}
              </span>
              <div
                className={`px-5 py-4 bg-[#1c1c1e] border rounded-2xl rounded-tl-sm flex items-center gap-3 w-fit shadow-lg ${isThinkingMode ? 'border-indigo-500/30 shadow-indigo-900/20' : 'border-white/5 shadow-cyan-900/10'}`}
              >
                <Loader2
                  size={16}
                  className={`animate-spin ${isThinkingMode ? 'text-indigo-400' : 'text-cyan-500'}`}
                />
                <span
                  className={`text-xs font-medium tracking-wide ${isThinkingMode ? 'text-indigo-200' : 'text-slate-400'}`}
                >
                  {isThinkingMode
                    ? '深度思考中...'
                    : isStoryboardActive
      ...
      <div className="p-4 bg-[#1c1c1e] border-t border-white/5 shrink-0 flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                ...
                setIsStoryboardActive(false);
                setIsHelpMeWriteActive(false);
              }}
            ...
            >
              <Brain
                size={12}
                className={isThinkingMode ? 'animate-pulse' : ''}
              />
              <span>深度思考模式</span>
            </button>
            ...
            <button
              onClick={() => {
                setIsStoryboardActive(!isStoryboardActive);
            ...
            <button
              onClick={() => {
                setIsHelpMeWriteActive(!isHelpMeWriteActive);
                setIsThinkingMode(false);
                setIsStoryboardActive(false);
              }}
          ...
          {isThinkingMode && (
            <span className="text-[9px] text-indigo-400/70 font-mono tracking-tight">
              Gemini 3 Pro
            </span>
          )}
        </div>
