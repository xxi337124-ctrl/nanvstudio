...
export const SidebarDock: React.FC<SidebarDockProps> = ({
  ...
  const [activePanel, setActivePanel] = useState<
    'history' | 'workflow' | 'add' | null
  >(null);
  const [activeHistoryTab, setActiveHistoryTab] = useState<'image' | 'video'>(
    'image'
  );
  ...
  const renderPanelContent = () => {
    if (activePanel === 'history') {
      const filteredAssets = assetHistory.filter(a => {
        if (activeHistoryTab === 'image')
          return (
            a.type === 'image' ||
            a.type.includes('image') ||
            a.type.includes('image_generator')
          );
        if (activeHistoryTab === 'video')
          return a.type === 'video' || a.type.includes('video');
        return false;
      });
            ...
            {filteredAssets.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-slate-500 opacity-60 select-none">
                ...
                <span className="text-[10px] font-medium tracking-widest uppercase">
                  暂无{activeHistoryTab === 'image' ? '图片' : '视频'}
                </span>
              </div>
            ) : (
                  ...
                  <div className="flex items-center justify-between px-1">
                    {editingWorkflowId === wf.id ? (
                      <input
                        ...
                        autoFocus
                        onBlur={e => {
                          onRenameWorkflow(wf.id, e.target.value);
                          setEditingWorkflowId(null);
                        }}
                        onKeyDown={e => {
                          if (e.key === 'Enter') {
                            onRenameWorkflow(wf.id, e.currentTarget.value);
                            setEditingWorkflowId(null);
                          }
                        }}
                      />
      ...
      >
        {[
          ...
          { id: 'workflow', icon: FolderHeart },
          {
            id: 'smart_sequence',
            icon: Clapperboard,
            action: onToggleMultiFrame,
            active: isMultiFrameOpen,
          },
          {
            id: 'sonic_studio',
            icon: Mic2,
            action: onToggleSonicStudio,
            active: isSonicStudioOpen,
            tooltip: '音频中心 (Audio Hub)',
          },
          ...
          <div key={item.id} className="relative group">
            <button
              ...
              onClick={() =>
                item.action ? item.action() : setActivePanel(item.id as any)
              }
            ...
            {(item.id === 'smart_sequence' || item.id === 'sonic_studio') && (
              <div className="absolute left-full ml-3 top-1/2 -translate-y-1/2 px-2 py-1 bg-black/80 backdrop-blur-md rounded border border-white/10 text-[10px] text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                {item.tooltip ||
                  (item.id === 'smart_sequence' ? '智能多帧' : '音频中心')}
              </div>
            )}
          </div>
        ))}
