...
interface NodeProps {
  ...
  onUpdate: (
    id: string,
    data: Partial<AppNode['data']> & { status?: NodeStatus },
...
const GLASS_PANEL =
  'bg-[#2c2c2e]/95 backdrop-blur-2xl border border-white/10 shadow-2xl';
...
const SecureVideo = ({
  ...
  if (error) {
    return (
      <div
        className={`flex items-center justify-center bg-zinc-800 text-xs text-red-400 ${className}`}
      >
        Load Error
      </div>
    );
...
const safePlay = (
  e: React.SyntheticEvent<HTMLVideoElement> | HTMLVideoElement
) => {
...
const InputThumbnails = ({
  ...
  const handleGlobalMouseUp = useCallback(
    (e: MouseEvent) => {
      ...
      const newIndex = Math.max(
        0,
        Math.min(originalAssets.length - 1, currentIndex + moveSlots)
      );
  ...
  return (
    <div
      className="flex items-center justify-center h-14 pointer-events-none select-none relative z-0"
      ...
      <div className="relative flex items-center gap-[6px]">
        {assets.map((asset, index) => {
          ...
          if (isItemDragging) {
            ...
            const draggingVirtualIndex = Math.max(
              0,
              Math.min(
                assets.length - 1,
                originalIndex + Math.round(dragOffset / ITEM_FULL_WIDTH)
              )
            );
            if (index > originalIndex && index <= draggingVirtualIndex)
              translateX = -ITEM_FULL_WIDTH;
            else if (index < originalIndex && index >= draggingVirtualIndex)
              translateX = ITEM_FULL_WIDTH;
          }
          ...
          return (
            ...
            >
              {isVideo ? (
                <SecureVideo
                  src={asset.src}
                  className="w-full h-full object-cover pointer-events-none select-none opacity-80 group-hover:opacity-100 transition-opacity bg-zinc-900"
                  muted
                  loop
                  autoPlay
                />
...
const NodeComponent: React.FC<NodeProps> = ({
  ...
  const commitPrompt = () => {
    if (localPrompt !== (node.data.prompt || ''))
      onUpdate(node.id, { prompt: localPrompt });
  };
  ...
  const handleInputResizeStart = (e: React.MouseEvent) => {
    ...
    inputStartDragY.current = e.clientY;
    inputStartHeight.current = inputHeight;
    const handleGlobalMouseMove = (e: MouseEvent) => {
      ...
      setInputHeight(
        Math.max(
          48,
          Math.min(
            inputStartHeight.current + (e.clientY - inputStartDragY.current),
            300
          )
        )
      );
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      if (node.data.videoUri.startsWith('data:')) {
    ...
    return () => {
      if (
        ...
        (mediaRef.current instanceof HTMLVideoElement ||
          mediaRef.current instanceof HTMLAudioElement)
      ) {
        try {
  ...
  const handleMouseEnter = () => {
    ...
    if (
      node.data.images?.length > 1 ||
      (node.data.videoUris && node.data.videoUris.length > 1)
    )
    ...
    if (onExpand && mediaRef.current) {
      ...
      if (node.type.includes('IMAGE') && node.data.image) {
        onExpand({
          ...
          initialIndex: (node.data.images || [node.data.image]).indexOf(
            node.data.image
          ),
        });
        ...
        const videos =
          node.data.videoUris && node.data.videoUris.length > 0
            ? node.data.videoUris
            : [src];
        const currentIndex = node.data.videoUris
          ? node.data.videoUris.indexOf(node.data.videoUri)
          : 0;
  ...
  const handleDownload = (e: React.MouseEvent) => {
    ...
    a.href = node.data.image || videoBlobUrl || node.data.audioUri || '';
    a.download = `nova-studio-${Date.now()}`;
  ...
  const handleAspectRatioSelect = (newRatio: string) => {
    ...
    const newSize: { width?: number; height?: number } = { height: undefined };
    if (w && h) {
  ...
  const handleTitleSave = () => {
    ...
    if (tempTitle.trim() && tempTitle !== node.title)
      onUpdate(node.id, {}, undefined, tempTitle);
    else setTempTitle(node.title);
  };
  ...
  const getNodeHeight = () => {
    ...
    const extra =
      node.type === NodeType.VIDEO_GENERATOR && generationMode === 'CUT'
        ? 36
        : 0;
  ...
  const renderTopBar = () => {
    ...
    return (
      <div
        className={`absolute -top-10 left-0 w-full flex items-center justify-between px-1 transition-all duration-300 ${showTopBar ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none'}`}
      >
        <div className="flex items-center gap-1.5 pointer-events-auto">
          {node.type === NodeType.VIDEO_GENERATOR && (
            <VideoModeSelector
              currentMode={generationMode}
              onSelect={mode => onUpdate(node.id, { generationMode: mode })}
            />
          )}
        ...
        <div className="flex items-center gap-2 pointer-events-auto">
          ...
          <div className={`px-2 py-1 flex items-center gap-2`}>
            {isEditingTitle ? (
              <input
                className="bg-transparent border-none outline-none text-slate-400 text-[10px] font-bold uppercase tracking-wider w-24 text-right"
                value={tempTitle}
              ...
              <span
                className="text-[10px] font-bold uppercase tracking-wider text-slate-400 hover:text-slate-200 cursor-text text-right"
      ...
      return (
        <div className="w-full h-full p-6 flex flex-col group/text">
          <div className="flex-1 bg-black/10 rounded-2xl border border-white/5 p-4 relative overflow-hidden backdrop-blur-sm transition-colors group-hover/text:bg-black/20">
            <textarea
              className="w-full h-full bg-transparent resize-none focus:outline-none text-sm text-slate-200 placeholder-slate-500 font-medium leading-relaxed custom-scrollbar selection:bg-amber-500/30"
    ...
    if (node.type === NodeType.VIDEO_ANALYZER) {
      return (
        <div className="w-full h-full p-5 flex flex-col gap-3">
          <div
            className="relative w-full h-32 rounded-xl bg-black/20 border border-white/5 overflow-hidden flex items-center justify-center cursor-pointer hover:bg-black/30 transition-colors group/upload"
            onClick={() => !node.data.videoUri && fileInputRef.current?.click()}
          >
            {videoBlobUrl ? (
              <video
                src={videoBlobUrl}
              ...
              <div className="flex flex-col items-center gap-2 text-slate-500 group-hover:upload:text-slate-300">
                <Upload size={20} />
            ...
            {node.data.videoUri && (
              <button
                className="absolute top-2 right-2 p-1 bg-black/50 rounded-full text-slate-400 hover:text-white backdrop-blur-md"
                onClick={e => {
                  e.stopPropagation();
          ...
          <div className="flex-1 bg-black/10 rounded-xl border border-white/5 overflow-hidden relative group/analysis">
            <textarea
              className="w-full h-full bg-transparent p-3 resize-none focus:outline-none text-xs text-slate-300 font-mono leading-relaxed custom-scrollbar select-text placeholder:italic placeholder:text-slate-600"
              value={node.data.analysis || ''}
              placeholder="等待分析结果，或在此粘贴文本..."
              onChange={e => onUpdate(node.id, { analysis: e.target.value })}
              onWheel={e => e.stopPropagation()}
              onMouseDown={e => e.stopPropagation()}
            ...
            {node.data.analysis && (
              <button
                className="absolute top-2 right-2 p-1.5 bg-black/60 hover:bg-black/80 border border-white/10 rounded-md text-slate-400 hover:text-white transition-all opacity-0 group-hover/analysis:opacity-100 backdrop-blur-md z-10"
                onClick={e => {
                  e.stopPropagation();
                  navigator.clipboard.writeText(node.data.analysis || '');
                }}
                title="复制全部"
              >
                <Copy size={12} />
          ...
          {node.data.audioUri ? (
            <div className="flex flex-col items-center gap-4 w-full z-10">
              ...
              <div className="flex items-center gap-4">
                <button
                  onClick={toggleAudio}
                  className="w-12 h-12 rounded-full bg-cyan-500/20 hover:bg-cyan-500/40 border border-cyan-500/50 flex items-center justify-center transition-all hover:scale-105"
                >
                  {isPlayingAudio ? (
            ...
            <div className="flex flex-col items-center gap-3 text-slate-600 z-10 select-none">
              {isWorking ? (
                <Loader2 size={32} className="animate-spin text-pink-500" />
              ) : (
                <Mic2 size={32} className="text-slate-500" />
              )}
      ...
      >
        {!hasContent ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-slate-600">
            <div
              className="w-20 h-20 rounded-[28px] bg-white/5 border border-white/5 flex items-center justify-center cursor-pointer hover:bg-white/10 hover:scale-105 transition-all duration-300 shadow-inner"
              onClick={() => fileInputRef.current?.click()}
            >
              {isWorking ? (
                <Loader2 className="animate-spin text-cyan-500" size={32} />
              ) : (
                <NodeIcon size={32} className="opacity-50" />
              )}
            </div>
            <span className="text-[11px] font-bold uppercase tracking-[0.2em] opacity-40">
              {isWorking ? '处理中...' : '拖拽或上传'}
            </span>
            <input
              type="file"
          ...
          <>
            {node.data.image ? (
              <img
                ...
                onContextMenu={e =>
                  onMediaContextMenu?.(e, node.id, 'image', node.data.image!)
                }
              />
            ) : (
              <SecureVideo
                ...
                onContextMenu={(e: React.MouseEvent) =>
                  onMediaContextMenu?.(e, node.id, 'video', node.data.videoUri!)
                }
              ...
              <div className="absolute inset-0 bg-black/40 z-10 grid grid-cols-2 gap-2 p-2 animate-in fade-in duration-200">
                {node.data.images
                  ? node.data.images.map((img, idx) => (
                      <div
                        key={idx}
                        className={`relative rounded-lg overflow-hidden cursor-pointer border-2 bg-zinc-900 ${img === node.data.image ? 'border-cyan-500' : 'border-transparent hover:border-white/50'}`}
                        onClick={e => {
                          e.stopPropagation();
                  ...
                  : node.data.videoUris?.map((uri, idx) => (
                      <div
                        key={idx}
                        className={`relative rounded-lg overflow-hidden cursor-pointer border-2 bg-zinc-900 ${uri === node.data.videoUri ? 'border-cyan-500' : 'border-transparent hover:border-white/50'}`}
                        onClick={e => {
                          e.stopPropagation();
                      ...
                      >
                        {uri ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-white/5 text-xs text-slate-500">
                            Failed
                          </div>
                        )}
                      </div>
            ...
            {generationMode === 'CUT' && node.data.croppedFrame && (
              <div className="absolute top-4 right-4 w-24 aspect-video bg-black/80 rounded-lg border border-purple-500/50 shadow-xl overflow-hidden z-20 hover:scale-150 transition-transform origin-top-right opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <img
                  src={node.data.croppedFrame}
            ...
            {generationMode === 'CUT' &&
              ...
              hasInputs &&
              inputAssets?.some(a => a.src) && (
                <div className="absolute top-4 right-4 w-24 aspect-video bg-black/80 rounded-lg border border-purple-500/30 border-dashed shadow-xl overflow-hidden z-20 hover:scale-150 transition-transform origin-top-right flex flex-col items-center justify-center group/preview opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="absolute inset-0 bg-purple-500/10 z-10"></div>
                  {(() => {
                    const asset = inputAssets!.find(a => a.src);
                    if (asset?.type === 'video') {
                      return (
                        <SecureVideo
                          src={asset.src}
                          className="w-full h-full object-cover opacity-60 bg-zinc-900"
                          muted
                          autoPlay
                        />
                      );
                  ...
                  <span className="absolute z-20 text-[8px] font-bold text-purple-200 bg-black/50 px-1 rounded">
                    分镜参考
                  </span>
                </div>
              )}
        ...
        {node.type === NodeType.VIDEO_GENERATOR &&
          generationMode === 'CUT' &&
          (videoBlobUrl || node.data.videoUri) && (
  ...
  const renderBottomPanel = () => {
    ...
    if (node.type === NodeType.VIDEO_GENERATOR) {
      ...
      models = [
        { l: 'Voice Factory (Gemini 2.0)', v: 'gemini-2.5-flash-preview-tts' },
      ];
    } else {
      models = [
    ...
    return (
      ...
      >
        ...
        {hasInputs && onInputReorder && (
          <div className="w-full flex justify-center mb-2 z-0 relative">
            <InputThumbnails
              assets={inputAssets!}
              onReorder={newOrder => onInputReorder(node.id, newOrder)}
            />
          </div>
        )}
        {/* Glass Panel: Set strict Z-Index to higher layer to overlap thumbnails */}
        <div
          className={`w-full rounded-[20px] p-1 flex flex-col gap-1 ${GLASS_PANEL} relative z-[100]`}
          onMouseDown={e => e.stopPropagation()}
          onWheel={e => e.stopPropagation()}
        >
          <div className="relative group/input bg-black/10 rounded-[16px]">
            ...
            />
            <div
              className="absolute bottom-0 left-0 w-full h-3 cursor-row-resize flex items-center justify-center opacity-0 group-hover/input:opacity-100 transition-opacity"
              onMouseDown={handleInputResizeStart}
            >
              <div className="w-8 h-1 rounded-full bg-white/10 group-hover/input:bg-white/20" />
            </div>
          </div>
          <div className="flex items-center justify-between px-2 pb-1 pt-1 relative z-20">
            <div className="flex items-center gap-2">
              <div className="relative group/model">
                <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-white/5 cursor-pointer transition-colors text-[10px] font-bold text-slate-400 hover:text-cyan-400">
                  <span>
                    {models.find(m => m.v === node.data.model)?.l || 'AI Model'}
                  </span>
                  <ChevronDown size={10} />
                </div>
                <div className="absolute bottom-full left-0 pb-2 w-40 opacity-0 translate-y-2 pointer-events-none group-hover/model:opacity-100 group-hover/model:translate-y-0 group-hover/model:pointer-events-auto transition-all duration-200 z-[200]">
                  <div className="bg-[#1c1c1e] border border-white/10 rounded-xl shadow-xl overflow-hidden">
                    {models.map(m => (
                      <div
                        key={m.v}
                        onClick={() => onUpdate(node.id, { model: m.v })}
                        className={`px-3 py-2 text-[10px] font-bold cursor-pointer hover:bg-white/10 ${node.data.model === m.v ? 'text-cyan-400 bg-white/5' : 'text-slate-400'}`}
                      >
                        {m.l}
                      </div>
                    ))}
                      ...
                      <div className="bg-[#1c1c1e] border border-white/10 rounded-xl shadow-xl overflow-hidden">
                        {(node.type.includes('VIDEO')
                          ...
                          <div
                            ...
                            className={`px-3 py-2 text-[10px] font-bold cursor-pointer hover:bg-white/10 ${node.data.aspectRatio === r ? 'text-cyan-400 bg-white/5' : 'text-slate-400'}`}
                          >
                            {r}
                          </div>
                        ))}
                  ...
                  <div className="flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-white/5 cursor-pointer transition-colors text-[10px] font-bold text-slate-400 hover:text-cyan-400">
                    ...
                    <span>
                      {node.data.resolution ||
                        (node.type.includes('IMAGE') ? '1k' : '720p')}
                    </span>
                  </div>
                    ...
                    <div className="bg-[#1c1c1e] border border-white/10 rounded-xl shadow-xl overflow-hidden">
                      {(node.type.includes('IMAGE')
                        ...
                        <div
                          ...
                          className={`px-3 py-2 text-[10px] font-bold cursor-pointer hover:bg-white/10 ${node.data.resolution === r ? 'text-cyan-400 bg-white/5' : 'text-slate-400'}`}
                        >
                          {r}
                        </div>
                      ))}
                    ...
                    <div className="bg-[#1c1c1e] border border-white/10 rounded-xl shadow-xl overflow-hidden">
                      {(node.type.includes('IMAGE')
                        ...
                        <div
                          ...
                          className={`px-3 py-2 text-[10px] font-bold cursor-pointer hover:bg-white/10 ${((node.type.includes('IMAGE') ? node.data.imageCount : node.data.videoCount) || 1) === c ? 'text-cyan-400 bg-white/5' : 'text-slate-400'}`}
                        >
                          {c}
                        </div>
                      ))}
            ...
            <button
              ...
              disabled={isWorking}
              className={`relative flex items-center gap-2 px-4 py-1.5 rounded-[12px] font-bold text-[10px] tracking-wide transition-all duration-300 ${isWorking ? 'bg-white/5 text-slate-500 cursor-not-allowed' : 'bg-gradient-to-r from-cyan-500 to-blue-500 text-black hover:shadow-lg hover:shadow-cyan-500/20 hover:scale-105 active:scale-95'}`}
            >
              {isWorking ? (
  ...
  const renderNode = () => (
    ...
    >
      {renderTopBar()}
      <div
        className={`absolute -left-3 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border border-white/20 bg-[#1c1c1e] flex items-center justify-center transition-all duration-300 hover:scale-125 cursor-crosshair z-50 shadow-md ${isConnecting ? 'ring-2 ring-cyan-400 animate-pulse' : ''}`}
      ...
      <div
        className={`absolute -right-3 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border border-white/20 bg-[#1c1c1e] flex items-center justify-center transition-all duration-300 hover:scale-125 cursor-crosshair z-50 shadow-md ${isConnecting ? 'ring-2 ring-purple-400 animate-pulse' : ''}`}
      ...
      <div className="w-full h-full flex flex-col relative rounded-[24px] overflow-hidden bg-zinc-900">
      ...
      {renderBottomPanel()}
      <div
        className="absolute -bottom-3 -right-3 w-6 h-6 flex items-center justify-center cursor-nwse-resize text-slate-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100 z-50"
        onMouseDown={e => onResizeMouseDown(e, node.id, nodeWidth, nodeHeight)}
      >
  ...
  // 3D 相机角度控制盘 - 直接渲染到 Portal 中
  return renderNode();
};

/**
 * Custom comparison function for Node memoization
...
const arePropsEqual = (prevProps: NodeProps, nextProps: NodeProps): boolean => {
  ...
  if (
    ...
    prevProps.isGroupDragging !== nextProps.isGroupDragging ||
  ...
  // Deep check node object (compare by reference and key properties)
  ...
  if (prevNode !== nextNode) {
    ...
    // Check data changes (shallow compare)
    const prevData = prevNode.data;
    const nextData = nextNode.data;

    for (const key of Object.keys(prevData) as Array<keyof typeof prevData>) {
  ...
  // Check input assets array
  const prevInputs = prevProps.inputAssets;
  const nextInputs = nextProps.inputAssets;
...
// Enable memo with custom comparison for better performance
export const Node = React.memo(NodeComponent, arePropsEqual);
