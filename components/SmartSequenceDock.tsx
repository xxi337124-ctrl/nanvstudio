...
export const SmartSequenceDock: React.FC<SmartSequenceDockProps> = ({
  isOpen,
  onClose,
  onGenerate,
  onConnectStart,
  ...
  const totalDuration = frames.reduce(
    (acc, f, i) => (i < frames.length - 1 ? acc + f.transition.duration : acc),
    0
  );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !transitionModalRef.current.contains(event.target as Node)
      ) {
    ...
    if (e.dataTransfer.files?.length > 0) {
      ...
      if (files.length > 0) {
        const readers = files.map(
          (file: any) =>
            new Promise<SmartSequenceItem>(resolve => {
  ...
  // --- Transition Editor ---
  const openTransitionEditor = (
    id: string,
    transition: { duration: number; prompt: string }
  ) => {
  ...
  const saveTransition = () => {
    if (editingTransitionId) {
      setFrames(prev =>
        prev.map(f =>
          f.id === editingTransitionId
            ? {
                ...f,
                transition: { prompt: tempPrompt, duration: tempDuration },
              }
            : f
        )
      );
  ...
  const renderPlayer = () => (
    ...
    >
      ...
      <div
        className={`absolute top-3 left-3 flex gap-2 z-20 transition-opacity duration-200 ${isGenerating ? 'opacity-0' : 'opacity-0 hover:opacity-100 group-hover/player:opacity-100'}`}
      >
        <button
          onClick={e => {
            e.stopPropagation();
        ...
        {resultVideoUrl && (
          <a
            href={resultVideoUrl}
            download={`nova-studio-seq_${Date.now()}.mp4`}
      ...
      {isGenerating ? (
        ...
        <div className="relative w-full h-full">
          ...
          <img
            src={
              hoverIndex !== null && frames[hoverIndex]
                ? frames[hoverIndex].src
                : frames[0].src
            }
            ...
            >
              {isPlaying ? (
                ...
                <Play
                  ...
                  fill={resultVideoUrl ? 'currentColor' : 'none'}
                />
              )}
            </button>
          </div>
      ...
      >
        ...
        >
          {frames.map((f, i) => (
            <div
              key={f.id}
              className="flex-1 h-full relative border-r border-white/5 last:border-0 overflow-hidden"
            >
              <img
                src={f.src}
                className="w-full h-full object-cover opacity-30 grayscale group-hover/strip:opacity-50 transition-opacity"
              />
            </div>
          ))}
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute -top-3 left-0 -translate-y-full p-2 text-slate-400 hover:text-white bg-black/50 backdrop-blur rounded-full border border-white/10 transition-colors"
          >
            <X size={14} />
          </button>
                ...
                {index < frames.length - 1 && (
                  <div className="relative flex flex-col items-center justify-center w-6 shrink-0 z-10">
                    ...
                    <button
                      data-link-btn
                      onClick={() =>
                        openTransitionEditor(frame.id, frame.transition)
                      }
                      className={`
                    ...
                    <span className="text-[8px] text-slate-500 mt-1 font-mono tracking-tighter">
          ...
          >
            ...
            <div className="p-4 pt-0 flex flex-col gap-3">
              <div className="flex items-center gap-2 bg-[#09090b] border border-white/10 rounded-xl px-3 py-2">
                ...
                />
                <span className="text-[10px] font-mono text-slate-300 min-w-[24px] text-right">
                  {tempDuration}s
                </span>
              </div>
              <button
